...
const createTest = (componentName, testSeed) => {
  const processedSeed = testSeed.replace(/{{name}}/g, path.basename(componentName, '.html'));
...
const generateTests = ([files = [], existingTests = []] = []) => {
  ...
  return createTestDirectory(COMPONENT_PATH)
    .then(() => getTestSeed())
    .then(seed => {
      return Promise.all(components.map(component => createTest(component, seed)));
    });
